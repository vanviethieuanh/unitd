...
// AutomountBlock represents the [Automount] section of a systemd unit.
//
...
// and [Install] sections. The automount specific configuration options are configured in the
// [Automount] section.
//
// Automount units must be named after the automount directories they control. Example: the automount
// point must be configured in a unit file . For details about the escaping logic used to convert a
// file system path to a unit name see . Note that automount units cannot be templated, nor is it
// possible to add multiple names to an automount unit by creating symlinks to its unit file.
//
// For each automount unit file a matching mount unit file (see for details) must exist which is
// activated when the automount path is accessed. Example: if an automount unit is active and the user
// accesses the mount unit will be activated.
//
// Note that automount units are separate from the mount itself, so you should not set or for mount
// dependencies here. For example, you should not set or similar on network filesystems. Doing so may
// result in an ordering cycle.
//
