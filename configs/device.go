...
// Device represents the Device configuration file of a systemd unit.
// A separate [Device] section does not exist.
//
...
// tag (by default all block and network devices, and a few others). Note that .
//
// Device units are named after the and paths they control. Example: the device is exposed in systemd
// as . For details about the escaping logic used to convert a file system path to a unit name see .
//
// To tag a udev device, use in the udev rules file, see for details.
//
// Device units will be reloaded by systemd whenever the corresponding device generates a event. Other
// units can use to react to that event.
type Device struct {
