...
// MountBlock represents the [Mount] section of a systemd unit.
//
...
// and [Install] sections. The mount specific configuration options are configured in the [Mount]
// section.
//
// Additional options are listed in , which define the execution environment the program is executed
// in, and in , which define the way the processes are terminated, and in , which configure resource
...
// multiple names to a mount unit by creating symlinks to its unit file.
//
// Optionally, a mount unit may be accompanied by an automount unit, to allow on-demand or parallelized
// mounting. See .
//
// Mount points created at runtime (independently of unit files or ) will be monitored by systemd and
// appear like any other mount unit in systemd. See description in .
//
// Some file systems have special semantics as API file systems for kernel-to-userspace and
// userspace-to-userspace interfaces. Some of them may not be changed via mount units, and cannot be
