...
// ScopeBlock represents the [Scope] section of a systemd unit.
//
...
// units manage externally created processes, and do not fork off processes on its own.
//
...
//
// See the for an introduction on how to make use of scope units from programs.
//
...
// process, but to the existence of at least one process in the scope. This also means that the exit
// statuses of these processes are not relevant for the scope unit failure state. Scope units may still
// enter a failure state, for example due to resource exhaustion or stop timeouts being reached, but
// not due to programs inside of them terminating uncleanly. Since processes managed as scope units
// generally remain children of the original process that forked them off, it is also the job of that
